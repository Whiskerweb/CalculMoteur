// app/server.ts
//
// Createur d'estimation — serveur local du skill calcul-moteur.
//
// STRICTEMENT LOCAL : ecoute sur 127.0.0.1, aucune authentification, aucun
// rate limit. Ne jamais exposer sur un reseau : quiconque l'atteint depense
// le credit OpenRouter.
//
// USAGE :
//   deno run --allow-env --allow-net --allow-read --env-file=.env app/server.ts
//   puis http://127.0.0.1:8788/

import { buildUserMessage, type FormInput, getSystemPrompt } from "./lib/prompt.ts";
import {
  DEFAULT_MODEL,
  getPreset,
  MODELS,
...
import { callOpenRouter, type ContentPart, type Message } from "./lib/openrouter.ts";
import { extractJson } from "./lib/extract.ts";
import { coerce, loadSchema, validate } from "./lib/validate.ts";
import { applyRecompute, recompute } from "./lib/totaux.ts";
import { applyCorrections } from "./lib/controle.ts";
...
const HTML_PATH = new URL("./ui.html", import.meta.url).pathname;
...
function handleHealth(): Response {
  ...
  return Response.json({
    ...
    skillError,
    schemaOk,
    keyPresent: Boolean(Deno.env.get("OPENROUTER_API_KEY")),
...
async function handler(req: Request): Promise<Response> {
  ...
  if (req.method === "GET" && (url.pathname === "/" || url.pathname === "/index.html")) {
    try {
      // Relu a chaque requete : editer ui.html + rafraichir, sans redemarrage.
